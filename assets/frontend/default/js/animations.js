function initializeAnimations(){
    document.querySelectorAll('.animation_container').forEach((element, key) => {
        if(isElementPartiallyVisible(element)){
            containerIsVisible(element);
        }else{
            inViewportPartially(element, (isVisible) => {
                if(isVisible){
                    containerIsVisible(element);
                }
            });
    ...
    function containerIsVisible(container) {
        container.querySelectorAll('.animate__animated').forEach((element, key) => {
            if(element.classList.contains('animate__animated')){
                animar(element, key);
            }
        })
    }
    function animar(element, key){
        setTimeout(() => {
            animateCSS(element, element.dataset.animation || 'fadeIn');
        }, key * 200);
    }
}

const animateCSS = (element = null, animation, prefix = 'animate__') =>{
...
// or
//animateCSS('.my-element', 'bounce').then((message) => {// Do something after the animation});
